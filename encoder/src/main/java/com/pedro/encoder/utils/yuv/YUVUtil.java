...
public class YUVUtil {
  ...
  /**
   * Copy pasted ARGBtoYUV420SemiPlanar with small change.
   * Only difference is that the B=? and R=? values are swapped
   */
  public static byte[] RGBAtoYUV420SemiPlanar(int[] input, int width, int height) {
    /*
     * COLOR_FormatYUV420SemiPlanar is NV12
     */
    final int frameSize = width * height;
    ...
    for (int j = 0; j < height; j++) {
      for (int i = 0; i < width; i++) {
        ...
        a = (input[index] & 0xff000000) >> 24; // a is not used obviously
        B = (input[index] & 0xff0000) >> 16;
        G = (input[index] & 0xff00) >> 8;
        R = (input[index] & 0xff) >> 0;
